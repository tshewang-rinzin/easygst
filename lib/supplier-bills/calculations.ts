...
export interface BillItemCalculated {
  subtotal: string;
...
/**
 * Calculate totals for a single bill line item
 */
export function calculateBillItem(item: BillItemInput): BillItemCalculated {
  ...
  // Calculate tax amount (0 if tax exempt) - INPUT GST
...
/**
 * Calculate bill totals from all line items
 */
export function calculateBillTotals(items: BillItem[]): BillTotals {
  ...
  // Track tax by rate for breakdown (Input GST)
  const taxByRate: Map<
    string,
