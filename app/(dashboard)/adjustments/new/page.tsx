...
export default function NewAdjustmentPage() {
  ...
  const getAdjustmentHelp = () => {
    switch (adjustmentType) {
      case 'discount':
        return 'Enter a negative amount (e.g., -50.00) to reduce the invoice total';
      case 'late_fee':
        return 'Enter a positive amount (e.g., 25.00) to add a late payment fee';
      case 'credit_note':
        return 'Enter a negative amount to credit the customer';
      case 'debit_note':
    ...
    <section className="flex-1 p-4 lg:p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">
          Add discounts, late fees, or other charges to an invoice
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <Label htmlFor="invoiceId" className="mb-2">
              ...
              >
                ...
                {invoices?.map((invoice: any) => (
                  <option key={invoice.id} value={invoice.id}>
                    {invoice.invoiceNumber} - {invoice.customer?.name} ({invoice.currency}{' '}
                    {parseFloat(invoice.amountDue).toFixed(2)} due)
                  </option>
                ))}
              </select>
              <p className="text-xs text-gray-500 mt-1">
                Select the invoice to apply this adjustment to
              </p>
            </div>
