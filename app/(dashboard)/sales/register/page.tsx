...
export default function SalesRegisterPage() {
  ...
  const [summary, setSummary] = useState<SalesRegisterSummary | null>(null);
  ...
  const fetchSalesRegister = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/sales-register?${params}`);
  ...
  const exportToCSV = () => {
    ...
    const rows = entries.map((entry) => [
      ...
      entry.type === 'invoice' ? 'Tax Invoice' : 'Cash Sale',
      entry.number,
      entry.customerName,
      entry.taxableAmount,
    ...
    a.href = url;
    a.download = `sales-register-${startDate.toISOString().split('T')[0]}-to-${endDate.toISOString().split('T')[0]}.csv`;
    ...
    <section className="flex-1 p-4 lg:p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">
          Comprehensive record of all sales transactions with GST breakdown
        </p>
      </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">
                  BTN {summary.totalSales.toFixed(2)}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  GST: BTN {summary.standardRated.gst.toFixed(2)}
                </p>
              </div>
              <div>
                ...
                <p className="text-xs text-gray-500">
                  GST: BTN {summary.zeroRated.gst.toFixed(2)}
                </p>
              </div>
              <div>
                ...
                <p className="text-xs text-gray-500">
                  GST: BTN {summary.exempt.gst.toFixed(2)}
                </p>
              </div>
              <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Sales Transactions ({entries.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            <div className="py-12 text-center">
              <p className="text-gray-500">Loading sales register...</p>
            </div>
          ) : entries.length === 0 ? (
            <div className="py-12 text-center">
