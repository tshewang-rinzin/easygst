...
import { getDebitNotes } from '@/lib/debit-notes/queries';

function DebitNotesSkeleton() {
  return (
    <Card>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {debitNotes.map(({ debitNote, supplier, bill }) => (
                <tr key={debitNote.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      <p className="font-medium text-gray-900">{debitNote.debitNoteNumber}</p>
                      {bill && (
                        <p className="text-xs text-gray-500">Ref: {bill.billNumber}</p>
                      )}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <Link href={`/purchases/debit-notes/${debitNote.id}`}>
  ...
  return (
    <section className="flex-1 p-4 lg:p-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-lg lg:text-2xl font-medium text-gray-900">Debit Notes</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage adjustments and reductions for supplier bills
          </p>
        </div>
        <Link href="/purchases/debit-notes/new">
          <Button className="bg-orange-500 hover:bg-orange-600">
      ...
      <Suspense fallback={<DebitNotesSkeleton />}>
        <DebitNotesList />
      </Suspense>
    </section>
  );
