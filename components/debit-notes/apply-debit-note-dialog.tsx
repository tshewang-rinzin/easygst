'use client';

import { useState, useTransition, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { Button } from '@/components/ui/button';
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from '@/components/ui/dialog';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '@/components/ui/select';
import { CreditCard, Loader2 } from 'lucide-react';
import { applyDebitNote } from '@/lib/debit-notes/actions';

interface Bill {
  id: string;
  billNumber: string;
  amountDue: string;
  currency: string;
}

interface ApplyDebitNoteDialogProps {
  debitNoteId: string;
  debitNoteNumber: string;
  supplierId: string;
  currency: string;
  unappliedAmount: string;
}

export function ApplyDebitNoteDialog({
  debitNoteId,
  debitNoteNumber,
  supplierId,
  currency,
  unappliedAmount,
}: ApplyDebitNoteDialogProps) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState('');
  const [bills, setBills] = useState<Bill[]>([]);
  const [loadingBills, setLoadingBills] = useState(false);
  const [selectedBillId, setSelectedBillId] = useState('');
  const [amount, setAmount] = useState('');

  // Fetch unpaid bills for this supplier when dialog opens
  useEffect(() => {
    if (open && supplierId) {
      setLoadingBills(true);
      fetch(`/api/suppliers/${supplierId}/outstanding-bills`)
        .then((res) => res.json())
        .then((data) => {
          setBills(data || []);
          setLoadingBills(false);
        })
        .catch(() => {
          setLoadingBills(false);
        });
    }
  }, [open, supplierId]);

  const selectedBill = bills.find((bill) => bill.id === selectedBillId);
  const maxAmount = Math.min(
    parseFloat(unappliedAmount),
    selectedBill ? parseFloat(selectedBill.amountDue) : 0
  );

  const handleApply = async () => {
    if (!selectedBillId) {
      setError('Please select a bill');
      return;
    }

    const applyAmount = parseFloat(amount);
    if (isNaN(applyAmount) || applyAmount <= 0) {
      setError('Please enter a valid amount');
      return;
    }

    if (applyAmount > maxAmount) {
      setError(`Amount cannot exceed ${currency} ${maxAmount.toFixed(2)}`);
      return;
    }

    setError('');

    startTransition(async () => {
      const formData = new FormData();
      formData.append('debitNoteId', debitNoteId);
      formData.append('billId', selectedBillId);
      formData.append('amount', amount);

      const result = await applyDebitNote({}, formData);

      if ('error' in result && result.error) {
        setError(result.error);
      } else if ('success' in result && result.success) {
        setOpen(false);
        setSelectedBillId('');
        setAmount('');
        router.refresh();
      }
    });
  };

  const handleBillSelect = (billId: string) => {
    setSelectedBillId(billId);
    const bill = bills.find((b) => b.id === billId);
    if (bill) {
      // Set amount to the lesser of unapplied amount or bill amount due
      const suggestedAmount = Math.min(
        parseFloat(unappliedAmount),
        parseFloat(bill.amountDue)
      );
      setAmount(suggestedAmount.toFixed(2));
    }
  };

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" className="bg-green-600 hover:bg-green-700">
          <CreditCard className="h-4 w-4 mr-2" />
          Apply to Bill
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Apply Debit Note</DialogTitle>
          <DialogDescription>
            Apply {debitNoteNumber} to an unpaid bill.
            <br />
            Available balance: {currency} {parseFloat(unappliedAmount).toFixed(2)}
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <Label htmlFor="bill">Select Bill</Label>
            {loadingBills ? (
              <div className="flex items-center justify-center py-4">
                <Loader2 className="h-5 w-5 animate-spin text-gray-400" />
              </div>
            ) : bills.length === 0 ? (
              <p className="text-sm text-gray-500 py-2">
                No unpaid bills found for this supplier.
              </p>
            ) : (
              <Select value={selectedBillId} onValueChange={handleBillSelect}>
                <SelectTrigger>
                  <SelectValue placeholder="Select a bill" />
                </SelectTrigger>
                <SelectContent>
                  {bills.map((bill) => (
                    <SelectItem key={bill.id} value={bill.id}>
                      {bill.billNumber} - Due: {currency} {parseFloat(bill.amountDue).toFixed(2)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            )}
          </div>

          {selectedBill && (
            <div className="space-y-2">
              <Label htmlFor="amount">Amount to Apply</Label>
              <Input
                id="amount"
                type="number"
                step="0.01"
                min="0.01"
                max={maxAmount}
                value={amount}
                onChange={(e) => setAmount(e.target.value)}
                placeholder={`Max: ${currency} ${maxAmount.toFixed(2)}`}
              />
              <p className="text-xs text-gray-500">
                Maximum amount: {currency} {maxAmount.toFixed(2)}
              </p>
            </div>
          )}
        </div>

        {error && (
          <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
            <p className="text-sm text-red-600">{error}</p>
          </div>
        )}

        <DialogFooter>
          <Button
            type="button"
            variant="outline"
            onClick={() => setOpen(false)}
            disabled={isPending}
          >
            Cancel
          </Button>
          <Button
            type="button"
            onClick={handleApply}
            disabled={isPending || !selectedBillId || !amount}
            className="bg-green-600 hover:bg-green-700"
          >
            {isPending ? (
              <>
                <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                Applying...
              </>
            ) : (
              'Apply Debit'
            )}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
