...
export function InvoiceFormNew({ defaultGstRate }: { defaultGstRate: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Remove the indexed item fields and reconstruct as proper array
    ...
    // Append items as JSON string (will be parsed on server)
                ...
                {lineItems.map((item, index) => (
                  <tr key={item.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-2 text-center">
                      <input
                        type="checkbox"
                        id={`items.${index}.isTaxExempt`}
                        name={`items.${index}.isTaxExempt`}
